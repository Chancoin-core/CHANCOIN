...
const std::string strMessageMagic = "Chancoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        // max doughies in the gravel at lidcombe stayshe
	for (const CTxIn txin : tx.vin) {
          LogPrintf("* saw this txid %s \n", txin.prevout.hash.ToString().c_str());
	  if (
		...
		(txin.prevout.hash == uint256S("982c8f54afab90b5163a7c7e845c05309427ea020a78afc635fe168e9759e9d5") && txin.prevout.n == 1))  // 99999.00000000 CHAN burned
                { 	return state.DoS(100, false, REJECT_INVALID, "CTransaction::CheckTransaction() : burnt coins unspendable");       }
	}
        ...
        if (fLimitFree && nModifiedFees < ::minRelayTxFee.GetFee(nSize)) {
            //Zero fee transations will be allowed until ❤
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s CHAN additional fees, %d delta bytes\n",
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    if (block.nVersion < VERSIONBITS_TOP_BITS && IsWitnessEnabled(pindexPrev, consensusParams))
        return state.Invalid(false, REJECT_OBSOLETE, strprintf("bad-version(0x%08x)", block.nVersion),
                                 strprintf("witness rejected nVersion=0x%08x block", block.nVersion));
